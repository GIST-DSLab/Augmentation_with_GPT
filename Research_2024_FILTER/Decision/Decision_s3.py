...
def generate_text(prompt):
    try:
        response = client.chat.completions.create(
            model = "generating_gpt35_turbo_16k",
...
candidate_folder_path = 'Generated_Data/Gen_S3'
selected_folder_path = 'Selected_Data/Selected_S3'
...
for file_name in prompt_file_names:
    ...
    if file_name.endswith('.json'):
        ...
        for i in range(len(candidate['train'])):
            input_s3_str += example + str(candidate['train'][i]) + prompt_question
...
save_json_file(input_s3_str, f'input_s3.json')
save_json_file(output_s3_str, f'output_s3.json')
